    ...
    return Table(
      ...
      children: [
        TableRow(
          ...
          children: [
            _buildHeaderCell("FROM POINT"),
            _buildHeaderCell("TO POINT"),
            _buildHeaderCell("X-COORD DIFF"),
            _buildHeaderCell("Y-COORD DIFF"),
            _buildHeaderCell("BEARING (DMS)"),
            _buildHeaderCell("DISTANCE (FEET)"),
          ],
        ),
        for (var row in tableData)
          TableRow(
            children: [
              _buildDataCell(row['from']),
              _buildDataCell(row['to']),
              _buildDataCell(row['xDiff']),
              _buildDataCell(row['yDiff']),
              _buildDataCell(row['bearing']),
              _buildDataCell(row['distance']),
            ],
          ),
      ],
    );
        ...
        build: (pw.Context context) => pw.Column(
          children: [
            ...
            pw.Table.fromTextArray(
              ...
              headers: ["FROM POINT", "TO POINT", "X-COORD DIFF", "Y-COORD DIFF", "BEARING (DMS)", "DISTANCE (FEET)"],
              data: _calculateTableData().map((row) {
                return [
                  row['from'],
                  row['to'],
                  row['xDiff'],
