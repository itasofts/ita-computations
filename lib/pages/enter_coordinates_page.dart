...
import 'package:printing/printing.dart'; // For PDF generation and printing
...
class EnterCoordinatesPageState extends State<EnterCoordinatesPage> {
  // Add the time tracker declaration here
            ...
            children: [
              Padding(
                ...
                child: Text(
                  ...
                  softWrap: true, // Wrap text when window is minimized
                ),
              ),
              Padding(
                padding: const EdgeInsets.all(8.0),
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          ...
                          child: Text("Paste & Add Rows"),
                        ),
                        ElevatedButton(
                          onPressed: _importCsv,
                          child: Text("Import CSV (N, E, L)"),
                        ),
                      ],
                    ),
                    SizedBox(height: 10),
  ...
  // Function to load and display FAQ from assets
  ...
  // Function to load and display EULA from assets
  ...
  // Method to paste coordinates
  void _pasteCoordinates() {
    List<String> lines = _pasteController.text.trim().split('\n');
    setState(() {
      for (String line in lines) {
        List<String> values = line.split(RegExp(r'[\s,]+')); // Split by space or comma
        if (values.length >= 2) {
          double x = double.tryParse(values[0]) ?? 0;
          double y = double.tryParse(values[1]) ?? 0;
          String beacon =
              values.length > 2 ? values.sublist(2).join(' ') : ''; // Join remaining parts as beacon
  ...
  // Method to build each coordinate row
        ...
        children: [
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(labelText: 'Beacon Label'),
            ),
          ),
          IconButton(
